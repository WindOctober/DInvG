...
void Location::initialize(int n, var_info *f, var_info * fd, var_info *fm,C_Polyhedron * p, string  name){
   ...
   this->d_cl=new Clump(fd, name, "Location"); // disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
...
void Location::add_clump(vector<Clump> & vcl){
   ...
   // ***
   tcout<<endl<<"> > > Location::"<<name<<" already has clump with "<<cl.get_count()<<" Polyhedra...";
   if (debug_2){
      tcout<<endl<<"- cl.print_clump(): "; cl.print_vector_of_poly();
   }
   //tcout<<"- The Root Context: "<<endl<<(*c)<<endl;
   c->recursive_strategy(cl);
   tcout<<endl<<"> > > Location::"<<name<<" altogether pushing back clump with "<<cl.get_count()<<" Polyhedra...";
   if (debug_2){
      tcout<<endl<<"- cl.print_clump(): "; cl.print_vector_of_poly();
   }
   vcl.push_back(cl);
...
ostream & operator << (ostream & in, Location const & l){
   ...
   in<<"「 l: "<<l.get_range_left()<<", n: "<<n<<", nd: "<<l.get_dual_var_info()->get_dimension()<<" 」"<<endl;
...
void Location::compute_dual_constraints(C_Polyhedron & initp) {
   ...
   tcout<<endl;
   tcout<<endl<<"> > > Location::compute_dual_constraints(), Location: "<<name<<"'s Initialization";
   ...
   if (p->is_empty()){
      ...
      tcout<<endl<<"< < < Initial-Value is empty in Location::"<<name;
   }
   else {
      tcout<<endl<<"- Initial-Value is not empty: ";
      tcout<<endl<<"  "<<*p;
   }
   if (!polyset) {
      tcout<<endl<<"< < < ( !polyset ) in Location::"<<name;
      return;
   }
   if (p->is_empty()){
      return;
   ...
   if (debug_2){
      tcout<<endl<<"------------------------------";
      tcout<<endl<<"- Farkas_Initialization_Poly (before projection): "<<endl<<"  "<<result.minimized_constraints();
   }
   ...
   //tcout<<endl<<"- The remove_higher_space_dimensions(in location) Time Taken is : "<<test_time_for_remove_higher_space_dimensions.compute_time_elapsed()<<endl;
   if (debug_2){
      tcout<<endl<<"------------------------------";
      tcout<<endl<<"- Farkas_Initialization_Poly (after projection): "<<endl<<"  "<<result.minimized_constraints();
   }
...
void Location::extract_invariant_from_generator(Generator const & g){
   ...
   if (debug_2){
      tcout<<endl<<"------------------------------";
      tcout<<endl<<"- generator: "<<g;
      if (g.is_point()){
         tcout<<endl<<"  g.is_point()";
      }
      else if (g.is_ray()){
         tcout<<endl<<"  g.is_ray()";
      }
      else if (g.is_line()){
         tcout<<endl<<"  g.is_line()";
      }
      else {
         tcout<<endl<<"  g is not point, not ray, not line";
      }
   }
   ...
   if (debug_2){
      tcout<<endl<<"- Location.invariant(before adding lin1): "<<endl<<"  "<<(*invariant);
      tcout<<endl<<"- lin1: "<<name<<" => "<<endl<<"  "; print_pure_lin_expression(lin1,f);
   }
   if (g.is_point() || g.is_ray()){
      ...
      if (flag){
         if (debug_2){
            tcout<<endl<<"- invariant->add_constraint(lin1>=0)";
         }
         ...
         if (debug_2){
            tcout<<endl<<"- invariant does not add_constraint(lin1>=0)";
         }
      }
   }
   else if (g.is_line()) {
   ...
   if (debug_2){
      tcout<<endl<<"- Location.invariant(after adding lin1): "<<endl<<"  "<<(*invariant);
   }
}

void Location::solve_invariant_from_generator(Generator const & g){
   //tcout<<endl<<"14.get here?";
   // Extract coefficients from l to r of the generators and make a constraint
   // Add this constraint to the invariant polyhedron

   //   tcout<<endl<<"1. old_invariant: "<<endl<<"  "<<invariant->minimized_generators();
   //   tcout<<endl<<"1. old_invariant: "<<endl<<"  "<<invariant->minimized_constraints();
   ...
   bool flag=true;
   for(int i=0;i<n;i++){
      if (! handle_integers(lin.coefficient(Variable(i)),c)){ // l+i turn to i
         flag = false;
      }
      lin1+= c * Variable(i);
   }
   ...
   //tcout<<endl<<"15.get here?";
   //   tcout<<endl<<"1. invariant: "<<endl<<"  "<<invariant->minimized_generators();
   //   tcout<<endl<<"1. invariant: "<<endl<<"  "<<invariant->minimized_constraints();
   if (debug_2){
      tcout<<endl<<"- Location.invariant(after adding lin1): "<<endl<<"  "<<(*invariant);
   }
}
...
void Location::solve_invariant_from_generator(Generator_System const & gs){
   ...
   //tcout<<endl<<"13.get here?";
}

void Location::extract_invariant_for_one_location_by_eliminating(Constraint_System const & pre_cs){

   if ( (int)(pre_cs.space_dimension()) == (n+1) ){
      C_Polyhedron Result(pre_cs);
      if (debug_2){
         tcout<<endl<<"------------------------------";
         tcout<<endl<<"- cpoly.constraints: "<<endl<<"  "<<Result.minimized_constraints();
         tcout<<endl<<"- cpoly.generators: "<<endl<<"  "<<Result.minimized_generators();
      }
      solve_invariant_from_generator(Result.minimized_generators());
      return;
   }

   //    Here is the function of "extract_invariant_for_one_location_by_eliminating()"
   //    It is similar to the code in 'Farkas' part.
   ...
   //tcout<<endl;
   //tcout<<endl<<"In extract_invariant_for_one_location_by_eliminating(), 「 "<<name<<", l: "<<l<<", n: "<<n<<", nd: "<<nd<<" 」";
   //contains_test(ph, n+1);
   // * * *
   ...
   // * * *
   Project(ph, l, l+(n+1) );
   //tcout<<endl<<"After eliminate by Projection, the poly is "<<endl<<"    "<<ph;

   if (debug_2){
      tcout<<endl<<"------------------------------";
      tcout<<endl<<"- cpoly(projected).constraints: "<<endl<<"  "<<ph.minimized_constraints();
      tcout<<endl<<"- cpoly(projected).generators: "<<endl<<"  "<<ph.minimized_generators();
   }
...
void Location::propagate_invariants_for_except_initial_by_propagation(C_Polyhedron & preloc_inv, C_Polyhedron & trans_rel){

   Constraint_System cs_preloc_inv = preloc_inv.minimized_constraints();
   C_Polyhedron ph = trans_rel;
   C_Polyhedron result;

   //ph.intersection_assign(loc_inv);// Aborted: terminate called after throwing an instance of 'std::invalid_argument', what():  PPL::C_Polyhedron::intersection_assign(y): this->space_dimension() == 4, y.space_dimension() == 2.
   ...
   */
   tcout<<endl<<"* Propagated Invariant at "<<name<<endl<<"  "<<*invariant;
}

void Location::extract_invariant_for_initial_by_recursive_eliminating(Constraint_System const & pre_cs){
   ...
   //    Here is the function of "extract_invariant_for_initial_by_recursive_eliminating()"
   //    It is similar to the code in 'Farkas' part.
   ...
   tcout<<endl<<"In extract_invariant_for_initial_by_recursive_eliminating(), 「 "<<name<<", l: "<<l<<", n: "<<n<<", nd: "<<nd<<" 」"<<endl;
   tcout<<"- - Currently, the number of variables in Ax<=b is : "<<pre_cs.space_dimension();
   ...
   //    1.Count the number of multiplier variables(that is generator y) required
   nc=0;
   for (vi=cs.begin();vi!=cs.end(); ++vi){
      nc++;
   }
   C_Polyhedron result(nc,UNIVERSE); // create a universe polyhedron of nc-dimensions
   Linear_Expression lin(0);
   ...
   for (i=0;i<cs.space_dimension();i++){
      if ( cs.space_dimension()-(n+1)<=i ){ // only select the coefficient about the last set of location
         lin=Linear_Expression(0);
         j=0;
         for (vi=cs.begin();vi!=cs.end(); ++vi){
   ...
   //tcout<<endl<<"- - - 2. y^T*A=0 is "<<endl<<"      "<<result;
   ...
   //    Now those are all the constraints.
   //tcout<<endl<<"- - - 3. y^T*A=0 (added some y>=0) is "<<endl<<"      "<<result;
   tcout<<endl<<"- - - 4. y^T*A=0 's minimized_constraints() is "<<endl<<"      "<<result.minimized_constraints();
   ...
   tcout<<endl<<"* * * The number of constraints(minimized) in 'y^T*A == 0': "<<test_nc1;
   tcout<<endl<<"* * * The number of variables in 'y': "<<"「 "<<test_cs.space_dimension()<<" 」";
   tcout<<endl<<"* * * The number of variables to be eliminated in 'A': "<<(n+1);
  
   //    5.After we get y^T*A=0, then transfer from the generator of y^T to values of y^T
   //tcout<<endl<<"- - - 5. y^T 's generators() is "<<endl<<"      "<<result1.generators();
   //tcout<<endl<<"- - - 6. y^T 's minimized_generators() is "<<endl<<"      "<<result1.minimized_generators();
   Generator_System gs = result1.minimized_generators();
   C_Polyhedron result2( cs.space_dimension()-(n+1) ,UNIVERSE); // Store y^T*b>=0
   for (vj=gs.begin(); vj!=gs.end(); vj++){
      ...
      //    here, 'b' contains the remained set of coefficient of C except the locations in 'A'.
      Linear_Expression lin1(0);
      j=0;
      for (vi=cs.begin(); vi!=cs.end(); ++vi){
         for (i=0;i<cs.space_dimension();i++){
            if (i<cs.space_dimension()-(n+1)){
               lin1=lin1 + y[j] * (*vi).coefficient(Variable(l+i)) * Variable(i);//second l+i turn to i
            }
         }
         j++;
      }
      result2.add_constraint(lin1 >= 0);
      //tcout<<endl<<"- - - 7. y^T*b>=0(added in the loop) is "<<endl<<result2<<endl;
      //    Now result2 contains the constraints of b( coefficient of invariant)
   }
   //tcout<<endl<<"- - - 8. y^T*b>=0 is "<<endl<<"      "<<result2;

   //    For minimized
   Constraint_System cs1 = result2.minimized_constraints();
   C_Polyhedron result3(cs1);
   tcout<<endl<<"- - - 9. y^T*b>=0 's minimized_constraints is "<<endl<<"      "<<result3;
   ...
   tcout<<endl<<"* * * The number of constraints(minimized) in 'y^T*b >= 0': "<<test_nc2;
   tcout<<endl<<"* * * The number of variables to be eliminated in 'b': "<<"「 "<<cs1.space_dimension()<<" 」"<<endl;
   //tcout<<endl<<"- - - 10. y^T*b>=0 's minimized_generators is "<<endl<<"      "<<result3.minimized_generators()<<endl;
   
   extract_invariant_for_initial_by_recursive_eliminating(result3.minimized_constraints());
...
void Location::extract_invariants_and_update(C_Polyhedron & pp, C_Polyhedron & dualp){
   tcout<<endl<<"For location: "<<name;
   tcout<<endl<<"「 l: "<<l<<", n: "<<n<<", nd: "<<fd->get_dimension()<<" 」";
   extract_invariant_from_generator(pp.minimized_generators());
   update_dual_constraints(dualp);
}

void Location::extract_invariants_and_update_for_one_location_by_eliminating(C_Polyhedron & pp, C_Polyhedron & dualp){
   //tcout<<endl<<"- In extract_invariants_and_update_for_one_location_by_eliminating()";
   //tcout<<endl<<"- Before extract_invariant_for_one_location_by_eliminating()";
   //tcout<<endl;
   int nd = fd->get_dimension();
   tcout<<endl<<"For location: "<<name;
   tcout<<endl<<"「 l: "<<l<<", n: "<<n<<", nd: "<<nd<<" 」";
   //tcout<<endl<<"C_Polyhedron & pp is "<<endl<<"    "<<pp;
   if (projection == "no_projection"){
      tcout<<endl<<"Do not use projection";
      if (debug_2){
         tcout<<endl<<"------------------------------";
         tcout<<endl<<"- cpoly.constraints: "<<endl<<"  "<<pp.minimized_constraints();
         tcout<<endl<<"- cpoly.generators: "<<endl<<"  "<<pp.minimized_generators();
      }
      extract_invariant_from_generator(pp.minimized_generators());
   }
   else {
      extract_invariant_for_one_location_by_eliminating(pp.minimized_constraints());
   }
   update_dual_constraints(dualp);
}

void Location::propagate_invariants_and_update_for_except_initial_by_propagation(C_Polyhedron & preloc_inv, C_Polyhedron & trans_rel/*, C_Polyhedron & dualp*/){
   tcout<<endl<<"= Doing Propagation at Location "<<name;
   tcout<<endl<<"= Location invariant "<<endl<<"  "<<preloc_inv;
   tcout<<endl<<"= Transition relation "<<endl<<"  "<<trans_rel;
   propagate_invariants_for_except_initial_by_propagation(preloc_inv, trans_rel);
   //update_dual_constraints(dualp);
}

void Location::contains_test(C_Polyhedron & pp, C_Polyhedron & loc_inv, C_Polyhedron & trans_rel){
   tcout<<endl<<"Start contains test";
   C_Polyhedron inv_extracted(invariant->space_dimension(), UNIVERSE);
   Generator_System gs = pp.minimized_generators();
   Generator_System::const_iterator vi=gs.begin();
   //tcout<<endl<<"l: "<<l<<", n: "<<n<<endl;
   for (;vi!=gs.end();vi++){
      //tcout<<(*vi)<<endl;
      Generator g = *vi;
      // Extract coefficients from l to r of the generators and make a constraint
   ...
   //tcout<<endl<<"ph.space_dimension: "<<ph.space_dimension();
   //tcout<<endl<<"After refine "<<endl<<"   "<<ph;
   Constraint_System cs = ph.minimized_constraints();
   ...
   Linear_Expression lin_inv(0);
   */
   //tcout<<endl<<"Invariant:"<<endl<<"   "<<*invariant;

   if (inv_extracted.contains(inv_propagated)){
      tcout<<endl<<"inv_extracted >= inv_propagated";
   }
   if (inv_propagated.contains(inv_extracted)){
      tcout<<endl<<"inv_extracted <= inv_propagated";
   }

   tcout<<endl<<"End contains test";
}

void Location::extract_invariants_and_update_for_initial_by_recursive_eliminating(C_Polyhedron & pp, C_Polyhedron & dualp){
   //tcout<<endl<<"- In extract_invariants_and_update_for_initial_by_recursive_eliminating()";
   //tcout<<endl<<"- Before extract_invariant_for_initial_by_recursive_eliminating()";
   //tcout<<endl<<"- - C_Polyhedron & pp is "<<endl<<"    "<<pp;
   //tcout<<endl<<"- - C_Polyhedron pp.constraints() is "<<endl<<"    "<<pp.constraints()<<endl;
   extract_invariant_for_initial_by_recursive_eliminating(pp.minimized_constraints());
   update_dual_constraints(dualp);
}

void Location::update_dual_constraints(C_Polyhedron & dualp){

   //tcout<<endl<<"> > > In update_dual_constraints(C_Polyhedron & dualp), Location : "<<name;
   ...
   //tcout<<endl<<"      "<<"After update_dual_constraints, dualp is ";
   //tcout<<endl<<"      "<<dualp;
   //tcout<<endl<<"< < < Out update_dual_constraints(C_Polyhedron & dualp), Location : "<<name<<endl;

}

void Location::extract_invariants_and_update_by_binary_eliminating(C_Polyhedron & pp, C_Polyhedron & dualp){
   //tcout<<endl<<"10.get here?";
   solve_invariant_from_generator(pp.minimized_generators());
   //tcout<<endl<<"11.get here?";
   update_dual_constraints(dualp);
}