...
LinExpr::LinExpr(int n, var_info *f) { initialize(n, f); }

void LinExpr::init_set(int n, var_info *f) { initialize(n, f); }
...
{
    LinExpr tmp(n, f);
    for (int i = 0; i < n + 1; i++) tmp[i] = lin[i] + p1(i);
...
{
    for (int i = 0; i < n + 1; i++) lin[i] += p1(i);
...
LinExpr operator*(int j, LinExpr const &p1)
{
    ...
    LinExpr tmp(n, f);
    for (int i = 0; i < n + 1; i++) tmp[i] = p1(i) * j;
    return tmp;  // A reference to tmp.lin will be copied.. so this is not all
                 // that costly
}
...
LinExpr operator*(LinExpr const &p1, Rational const &i)
{
    ...
    var_info *f = p1.get_info();
    LinExpr tmp(n, f);
    for (int j = 0; j < n + 1; j++) tmp[j] = i * p1(j);
    return tmp;  // A reference to tmp.lin will be copied.. so this is not all
                 // that costly
}
...
int LinExpr::get_dim() const { return n; }
var_info *LinExpr::get_info() const { return f; }
...
bool LinExpr::equiv(LinExpr const &l2, Rational &factor) const
{
    ...
    while (i < n + 1) {
        if (factor * lin[i] != l2(i)) return false;
        i++;
    }
    return true;
}
