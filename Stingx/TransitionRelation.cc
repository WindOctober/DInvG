...
void TransitionRelation::compute_consecution_constraints( Context & c){
   ...
   tcout<<endl;
   tcout<<endl<<"> > > (intra transition) In compute_consecution_constraints(), TransitionRelation : "<<name;
   Clump * preloc_d_cl = preloc->get_d_cl();
   bool compute_this_trans = true;
   tcout<<endl<<"Current transition has poly as follows: ";
   tcout<<endl<<"  "<<*rel;
   ...
   tcout<<endl<<"  Compute this transition: "<<compute_this_trans;
   ...
   if (compute_this_trans){
      ...
      if (debug_3){
         ...
         tcout<<endl<<"- l1: "<<l1;
         tcout<<endl<<"- l2: "<<l2;
         tcout<<endl<<"- offset1: "<<offset1;
         tcout<<endl<<"- offset2: "<<offset2;
      }

      if (debug_2){
         tcout<<endl<<"------------------------------";
         tcout<<endl<<"- vi=cs.begin()~end(): ";
         for (vi=cs.begin();vi!=cs.end();vi++){
            tcout<<endl<<"  "<<(*vi);
         }
      }
      ...
      for(i=0;i<n;i++){
         ...
         //tcout<<endl<<"1. ll in ll==0 is : "<<endl<<ll<<endl;
         //polyd.add_constraint_and_minimize(ll==0);
         polyd.add_constraint(ll==0);
         polyd_dis.add_constraint(ll==0); // disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
         if (debug_2){
            tcout<<endl<<"- unprimed constraint(in polyd_dis): "<<endl<<"  "<<(ll==0);
         }
      }
      ...
      for(i=0;i<n;i++){
         ...
         //tcout<<endl<<"2. ll in ll==0 is : "<<endl<<ll<<endl;
         //polyd.add_constraint_and_minimize(ll==0);
         polyd.add_constraint(ll==0);
      }
      for(i=0;i<n;i++){
      // disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
         ...
         if (debug_2){
            tcout<<endl<<"- primed constraint(in polyd_dis): "<<endl<<"  "<<(ll==0);
         }
      }
      ...
      polyd_dis.add_constraint(ll>=1);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
      if (debug_2){
         tcout<<endl<<"- constant constraint(in polyd_dis): "<<endl<<"  "<<(ll>=1);
      }
      ll+=Variable(offset1+n); // + d_preloc
      //tcout<<endl<<"3. ll in ll>=0 is : "<<endl<<ll<<endl;   
      //polyd.add_constraint_and_minimize(ll>=0) ;
      polyd.add_constraint(ll>=0) ;

      // (4) Now for the positivity constraint (or \lambda >= 0 or \forall \lambda)
      j=0;
      for (vi=cs.begin();vi!=cs.end(); ++vi){
         //tcout<<endl<<"Am I ready to place '>=0' to lambda ? "<<endl;
         if ((*vi).type()==Constraint::NONSTRICT_INEQUALITY){
            //tcout<<endl<<"Constraint is Non-Strict inequality"<<endl;
            //tcout<<endl<<"4. "<<endl<<Variable(offset2+j)<<">=0"<<endl;
            //polyd.add_constraint_and_minimize(Variable(offset2+j)>=0);
            polyd.add_constraint(Variable(offset2+j)>=0);
            polyd_dis.add_constraint(Variable(offset2+j)>=0);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
            if (debug_2){
               tcout<<endl<<"- positivity \\lambda constraint(in polyd_dis): "<<endl<<"  "<<(Variable(offset2+j)>=0);
            }
            ...
            //tcout<<endl<<"5. "<<endl<<Variable(offset2+j)<<">=0"<<endl;
            //polyd.add_constraint(Variable(offset2+j)>=0);
         } else {
      ...
      if (debug_2){
         tcout<<endl<<"------------------------------";
         tcout<<endl<<"- polyd_dis(with \\lambda): "<<endl<<"  "<<polyd_dis;
      }
      ...
      polyd_dis.remove_higher_space_dimensions(2*n+2);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
      // * * *
      // * * *
      ...
      //tcout<<endl<<"- The remove_higher_space_dimensions(in intra location) Time Taken is : "<<test_time_for_remove_higher_space_dimensions.compute_time_elapsed()<<endl;
      if (debug_2){
         tcout<<endl<<"- polyd_dis(projected away \\lambda): "<<endl<<"  "<<polyd_dis;
      }

      // now populate the context
      cs=polyd.minimized_constraints();
      cs_dis=polyd_dis.minimized_constraints();// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/20
      if (debug_2){
         tcout<<endl<<"- cs_dis(projected away \\lambda): "<<endl<<"  "<<cs_dis;
      }
      ...
      // mu=1 in disabled path
         ...
         if (debug_2){
            tcout<<endl<<"------------------------------";
            tcout<<endl<<"- ll1(0): "<<endl<<"  "<<ll1;
         }
         for (vi=cs_dis.begin();vi!=cs_dis.end();++vi){
            ll1*=0;
            if (debug_2){
               tcout<<endl<<"- vi=cs_dis.begin()~end(): "<<endl<<"  "<<(*vi);
            }
            for(i=0;i<=n;i++)
               ll1[l1+i]=handle_integers((*vi).coefficient(Variable(i)));
            if (debug_2){
               tcout<<endl<<"  ll1(l1): "<<endl<<"  "<<ll1;
               if ((*vi).is_inequality()){
                  tcout<<" >= 0 ";
               }
               else if ((*vi).is_equality()){
         ...
         if (debug_2){
            tcout<<endl<<"- pdis1(mu=1 in disabled path): "<<endl<<"  "<<pdis1;
         }
      }
      tcout<<endl<<"  pushing back clump with "<<preloc_d_cl->get_count()<<" Polyhedra (in falsepath)...";
   }

   tcout<<endl<<"< < < Intra-Transition::"<<name<<" prepare to push back clump with "<<preloc_d_cl->get_count()<<" Polyhedra";
   
#ifdef __DEBUG__D_
   tcout<<"Test Message from TransitionRelation::compute_constraints(Context &)"<<endl;
   tcout<<c;
   tcout<<"Message ends"<<endl<<endl;
#endif
}

void TransitionRelation::compute_consecution_01(vector<Clump> & vcl){
   tcout<<endl;
   tcout<<endl<<"> > > (inter transition) In compute_consecution_01(), TransitionRelation : "<<name;
   ...
   if (compute_this_trans){
      ...
      for(i=0;i<n;i++){
         ...
         //polyd.add_constraint_and_minimize(ll==0);
         polyd.add_constraint(ll==0);
         polyd_dis.add_constraint(ll==0);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/13
      }
      ...
      for(i=0;i<n;i++){
      // disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/13
         ll= Linear_Expression(0);
         j=0;
         for(vi=cs.begin();vi != cs.end(); vi++){
            ll+= handle_integers((*vi).coefficient(Variable(n+i))) * Variable(offset2+j); // coefficient for \lambda_j
      ...
      polyd_dis.add_constraint(ll>=1);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/13
      ...
      for (vi=cs.begin();vi!=cs.end(); ++vi){
         if ((*vi).type()==Constraint::NONSTRICT_INEQUALITY){
            //polyd.add_constraint_and_minimize(Variable(offset2+j)>=0);
            polyd.add_constraint(Variable(offset2+j)>=0);
            polyd_dis.add_constraint(Variable(offset2+j)>=0);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/13
         } else if ((*vi).type()==Constraint::STRICT_INEQUALITY){
            cerr<<"Location::compute_dual_constraints -- Warning: Encountered Strict Inequality"<<endl;
            cerr<<"                "<<(*vi)<<endl;
            //polyd.add_constraint_and_minimize(Variable(offset2+j)>0);
      ...
      // Old method for eliminate lambda
      polyd.remove_higher_space_dimensions(2*n+2);
      polyd_dis.remove_higher_space_dimensions(2*n+2);// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/13
      // * * *
      // * * *
      // New method for eliminate lambda: Farkas' Lemma 2.5a, Ax<=b
      //eliminate_by_Farkas(polyd, 2*n+2);
      ...
      // * * *
      //test_time_for_remove_higher_space_dimensions.stop();
      //tcout<<endl<<"- The remove_higher_space_dimensions(in inter location) Time Taken is : "<<test_time_for_remove_higher_space_dimensions.compute_time_elapsed()<<endl;
      if (debug_2){
         tcout<<endl<<"- polyd_dis(projected away \\lambda): "<<endl<<"  "<<polyd_dis;
      }
      ...
      cs=polyd.minimized_constraints();
      cs_dis=polyd_dis.minimized_constraints();// disabled path; added by Hongming, at Shanghai Jiao Tong University, 2022/09/13
      if (debug_2){
         tcout<<endl<<"- cs_dis(projected away \\lambda): "<<endl<<"  "<<cs_dis;
      }
      C_Polyhedron p0(nd,UNIVERSE), p1(nd,UNIVERSE), pdis0(nd,UNIVERSE), pdis1(nd,UNIVERSE);
      //Expression e(nd,r,fd,fm);
      LinExpr ll1(nd,fd);
      if (one){
      // mu=1
         ...
         if (debug_2){
            tcout<<endl<<"------------------------------";
            tcout<<endl<<"- p1(mu=1 in enabled path): "<<endl<<"  "<<p1;
         }
      }
      tcout<<endl<<"  pushing back clump with "<<cl.get_count()<<" Polyhedra...";
      if (zero){
      // mu=0
         ll1*=0;
         for (vi=cs.begin();vi!=cs.end();++vi){
         ...
         cl.insert(p0);
         if (debug_2){
            tcout<<endl<<"- p0(mu=0 in enabled path): "<<endl<<"  "<<p0;
         }
      }
      tcout<<endl<<"  pushing back clump with "<<cl.get_count()<<" Polyhedra...";
      if (falsepath){
      ...
      tcout<<endl<<"  pushing back clump with "<<cl.get_count()<<" Polyhedra (in falsepath)...";
   ...
   tcout<<endl<<"< < < Inter-Transition::"<<name<<" pushing back clump with "<<cl.get_count()<<" Polyhedra...";
   if (cl.get_count()!=0){
      vcl.push_back(cl);
   }
   tcout<<"done";
