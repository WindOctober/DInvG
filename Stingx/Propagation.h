...
// propagate the preloc's invariants into postloc's initial-value by projection or matrix-method
void propagate_from_inv_to_initval(C_Polyhedron & preloc_inv, C_Polyhedron & trans_relation, int postloc_index);

// propagate the p-transition obtained from the front element, from pre's invariant to post's invariant
...
// propagate the p-transition, which is obtained from the front element, from pre's invariants to post's initial-value
...
// return the initial-location-index which has initial-value and should have computed invariants
...
// all invariants are worked out, which means that there is no empty ppg_flag or there is no location need to compute propagation and Farkas
...
// return the location-index which has initial-value, ppged_flag and should be computed invariants by farkas
