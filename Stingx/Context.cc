...
void Context::Convert_CNF_to_DNF_and_Print(vector<Location *> * loclist, C_Polyhedron * dualp, int wtime, bool timed ){
   Timer one_timer;
...
void Context::Convert_CNF_to_DNF_and_Print(vector<Location *> * loclist, C_Polyhedron * dualp, int wtime, bool timed, Timer & one_timer){
  int i = 1;
  if ( timed && one_timer.compute_time_elapsed() >= wtime ){
    cerr <<"Time is up"<<endl;
    return;
  ...
  while( i > 0 ){
    ...
    if ( i > 0){
      ...
      child1->Convert_CNF_to_DNF_and_Print(loclist, dualp, wtime, timed, one_timer);
    } else{
      split_01_strategy(loclist, dualp, wtime, timed, one_timer);
      return;
    }
...
void Context::recursive_strategy(Clump & vp){
   ...
   while (i>0){
      ...
      if (i> 0){
         ...
         split_01_strategy(vp); //contains process "vp.insert(ps->get_poly_reference())";
         return;
      }
   }
