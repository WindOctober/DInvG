...
bool DualTransitionRelation::add_guard(Constraint const& cc) {
    ...
    if (cc.is_equality())
        ...
        INVARIANT(false,
                  "DualTransitionRelation::add_guard() -- should not reach "
...
void DualTransitionRelation::split_relation() {
    ...
    PRECONDITION(
        guard_.space_dimension() == (unsigned)(n_ + 1) &&
...
DualTransitionRelation::DualTransitionRelation(var_info* f,
    ...
    //   INVARIANT( OK__ (), "
    //   DualTransitionRelation::DualTransitionRelation(...) invariant failed
    //   ");
    split_relation();
}
