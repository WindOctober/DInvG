...
void Location::ComputeCoefConstraints(C_Polyhedron& initPoly) {
    ...
    if (poly->is_empty()) {
        ...
        cout << endl << "< < < Initial-Value is empty in Location::" << locName;
        return;
    }
    if (!initFlag) {
...
void Location::extract_invariant_for_initial_by_recursive_eliminating(
    ...
    if ((int)(pre_cs.space_dimension()) == (varsNum + 1)) {
        ...
        computeInvFromGenerator(Result.minimized_generators());
        return;
    }
    int i, j, constraintNum, coefNum;
    coefNum = coefInfo->getDim();
    ...
    C_Polyhedron result(constraintNum, UNIVERSE);
    Linear_Expression lin(0);
    ...
    for (vj = gs.begin(); vj != gs.end(); vj++) {
        Generator g = (*vj);
        vector<int> y(constraintNum, -999);
        ...
        for (it = constraints.begin(); it != constraints.end(); ++it) {
            for (i = 0; i < static_cast<int>(constraints.space_dimension());
                ...
                if (i < static_cast<int>(constraints.space_dimension()) -
                            (varsNum + 1)) {
                    lin1 = lin1 + y[j] *
                                      (*it).coefficient(Variable(LIndex + i)) *
                                      Variable(i);
                }
            }
            j++;
