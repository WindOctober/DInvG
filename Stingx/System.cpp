...
void System::add_location(Location *loc) { vloc.push_back(loc); }
...
{
    if (context_computed_) delete (glc_);
...
{
    if (!context_computed_) compute_initial_context();
...
bool System::location_matches(string name)
{
    vector<Location *>::iterator vi;
    for (vi = vloc.begin(); vi != vloc.end(); ++vi) {
        if ((*vi)->matches(name)) return true;
    }
    return false;
}
